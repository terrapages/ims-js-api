...
$.extend({
	...
	getUsersDataOfFriends: function (users, callback, errorCallback) {
		$.ajax({
			type : 'GET',
			url : apiContextURL + 'lv2users/getUsersDataOfFriends',
			data : '',
			dataType : 'text',
			beforeSend : function(xhr) {
				$.beforeSendRequest(xhr);
	...
	getUsersData: function (users, callback, errorCallback) {
		getUserDataInternal('getUserData', users, callback, errorCallback);
	},

	getAllUsersData: function (callback, errorCallback) {
		$.ajax({
			type: 'GET',
			url: apiContextURL + 'lv2users/getAllUsersData/',
			data: '',
			dataType: 'text',
			beforeSend: function (xhr) {
				$.beforeSendRequest(xhr);
	...
	authWebSocket: function (auth, callback) {
		$.state.socket.emit('authenticate',auth.person, auth ,appID);
	},
	...
	authenticateWithWebsockets: function (auth, callback) {
		$.state.socket.emit('authenticate',$.lv2.username, hashAuth(auth),Properties.APPLICATION_ID,serviceKey);

	},

	openWebsocket : function (onFriendLogInOut) {
		console.log("MED: " + Properties.MEDUSA_URL);

		$.state.socket = io.connect(Properties.MEDUSA_URL, {resource: 'nodejs'});
		...
		$.state.socket.on('user list', function(data) {
			...
			onFriendLogInOut(usersSignedIn);
		});

		console.log("SOK: " + $.state.socket);
		//When a user logs in or out we ask the server for an updated list of friends
		$.state.socket.on('user authenticated', function(data){
			$.state.socket.emit('get list', $.lv2.username, $.lv2.token);
		});
		$.state.socket.on('user disconnected', function(data){
			$.state.socket.emit('get list', $.lv2.username, $.lv2.token);
		});

		console.log("SK: " + serviceKey);
		console.log("UN: " + $.lv2.username);
		console.log("TK: " + $.lv2.token);
		$.state.socket.emit('authenticate IMS',$.lv2.username, Properties.APPLICATION_ID);
...
function getUsersDataInternal(endpoint, users, callback, errorCallback) {
	var ajaxCounter = 0;
	var totalData = [];
	var index = 0;
	...
	while (index < users.length) {
		...
		$.ajax({
			type: 'GET',
			url: apiContextURL + 'lv2users/' + endpoint + '/' + args,
			data: '',
			dataType: 'text',
			beforeSend: function (xhr) {
				$.beforeSendRequest(xhr);
...
function closeWebsocket() {
	if ($.state.socket != null) {
		$.state.socket.emit('log out', $.lv2.username, $.lv2.token, serviceKey);
		$.state.socket.close();
	}
}